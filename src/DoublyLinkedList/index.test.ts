...
describe("DoublyLinkedList tests 1", () => {
	...
	it("pushStart and chaining", () => {
		...
		expect(list.getHead()?.next?.data).toBe(3);
		expect(list.getHead()?.next?.next?.data).toBe(2);

		list.pushStart(5)
		expect(list.getSize()).toBe(5);
		expect(list.getHead()?.data).toBe(5);
		expect(list.getHead()?.next?.data).toBe(4);
		expect(list.getHead()?.next?.next?.data).toBe(3);
	});
...
describe("DoublyLinkedList tests 2", () => {
	...
	it("Delete index out of bounds index", () => {
		const list = new DoublyLinkedList<number>([11]);
